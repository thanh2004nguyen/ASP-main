	...
	$(document).ready(function () {
		...
		fetch('/Stationery/ChartData')
			...
			.then(data => {
				...
				var labels = dataPoints.map(point => point.productName);
			

				var labels = dataPoints.map(point => point.productName);
				var totalQuantity = dataPoints.map(point => point.totalQuantity);
				var percentQuantity = dataPoints.map(point => point.quantityAndEmp);

				console.log('Labels:', labels);
				console.log('Percentage Costs:', totalQuantity);
				console.log('Percentage Costs:', percentQuantity);
				...
				var basiclinechart = new Chart(ctx, {
					...
					data: {
						...
						datasets: [{
							...
							label: "Percent Quantity/Total Emp",
							fill: false,
							backgroundColor: '#fb9678',
							borderColor: '#fb9678',
							data: percentQuantity
						}]
					},
					options: {
						responsive: true,
