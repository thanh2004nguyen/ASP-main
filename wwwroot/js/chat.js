$(document).ready(function () {
   ...
   /*     // Add a computed property for each room to track unseen messages
        self.roomHasUnseenMessages = ko.computed(function () {
            var roomMessages = {};

            // Initialize the tracking for each room
            self.chatRooms().forEach(function (room) {
                roomMessages[room.id()] = ko.observable(false);
            });

            // Update the tracking based on messages
            self.chatMessages().forEach(function (message) {
                if (message.seen() === false) {
                    console.log("aa");
                    roomMessages[message.roomId()](false);
                }
        ...
        // Add a computed property to check if any room has unseen messages
        self.anyRoomHasUnseenMessages = ko.computed(function () {
            var roomHasUnseenMessages = self.roomHasUnseenMessages();
         

            for (var roomId in roomHasUnseenMessages) {
                if (roomHasUnseenMessages[roomId]()) {
                    return true;
                }
            }
        ...
        self.hasUnseenMessages = ko.computed(function () {
            var unseenMessages = {};

            ko.utils.arrayForEach(self.chatRooms(), function (room) {
                var roomMessages = ko.utils.arrayFilter(self.chatMessages(), function (message) {
                    return message.seen() === false;
                });

                unseenMessages[room.id()] = roomMessages.length > 0;
            });
            ...
            fetch('/api/Messages/Room/' + viewModel.joinedRoom().name())
                ...
                .then(data => {
                    ...
                    for (var i = 0; i < data.length; i++) {
                        ...
                        var message = new ChatMessage(data[i].id,
                            data[i].content,
                            data[i].timestamp,
                            data[i].fromUserName,
                            data[i].fromFullName,
    ...
    function ChatMessage(id, content, timestamp, fromUserName, fromFullName, isMine, avatar,seen) {
        var self = this;
        self.id = ko.observable(id);
        self.content = ko.observable(content);
        self.timestamp = ko.observable(timestamp);
        self.seen = ko.observable(seen || false);
